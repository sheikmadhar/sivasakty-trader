...
export class ProductDetailPage implements OnInit {
    ...
    for (let value of this.product.variations) {
      this.config.getWoo("products/" + value + "?" + this.config.productsArguments).then(data => {
        ...
        if (count == this.product.variations.length) {
          ...
          this.product.default_attributes.forEach((e, ind) => {
            this.fillAttributesIni(e.option, e.name, ind);
          });
        }
      });
    }
  ...
  initializeAllVariationAttributes(p) {
    ...
    console.log("initializeAllVariationAttributes" + this.allVariableAttributes);
  }
  ...
  sortAttributes() {
    ...
    console.log("sortAttributes" + this.tempAllVariableAttributes);
  }
  ...
  fillAttributesIni = function (val, key, position) {
    let count = 0;
    this.selectAttributes.forEach((value, index) => {
      if (value.position == position) { value.value = val; count++; }
      if (val == 'choose' && value.position == position) {
  ...
  //===============================================================================================================================
  selectVariationINi() {
    console.log("selectVariation" + this.tempAllVariableAttributes);
    let pId = null;
    ...
    console.log("selectVariation133" + JSON.stringify(this.selectAttributes));
    if (this.selectAttributes != null)
      console.log("selectVariation13");
    this.resetAttributes();
    this.updateProductDetail(this.selectedVariation);
    console.log("selectVariation4");
    this.resetAttributes();
  }
  selectVariation() {
    console.log("selectVariation" + this.tempAllVariableAttributes);
